...
const AirlineDetails = ({ booking, onDelete, canManage = true }) => {
  ...
  useEffect(() => {
    if (booking) {
      const airlineDocuments = booking.booking_documents.filter(doc => doc.document_for === 'airline');
  ...
  return (
    <div className="space-y-2 pb-2">
      <div className="flex justify-between items-center">
        <h2 className="text-base font-medium text-gray-500">Shared Airline Tickets Detail</h2>
        {canManage ? (
          <button
            onClick={handleEdit}
      ...
      <div className="bg-gray-50 rounded space-y-4">
        {booking.booking_airline_details.map((detail, index) => (
          <div key={index} className="p-4 bg-white border border-gray-200 rounded shadow-sm">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-700"><strong>Flight Date:</strong> {new Date(detail.flight_date).toLocaleString()}</p>
              </div>
              <div>
                <p className="text-sm text-gray-700"><strong>Flight From:</strong> {detail.flight_from}</p>
              </div>
              <div>
                <p className="text-sm text-gray-700"><strong>Flight Time:</strong> {detail.flight_time}</p>
              </div>
              <div>
                <p className="text-sm text-gray-700"><strong>Flight To:</strong> {detail.flight_to}</p>
              </div>
            </div>
          </div>
        ))}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {airlineDocuments.map((doc) => (
            <div key={doc.document_id} className="flex justify-between items-center p-4 bg-white border border-gray-200 rounded shadow-sm">
