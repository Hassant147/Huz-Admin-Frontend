...
const FeaturedPackages = () => {
  ...
  const handleToggleFeatured = async (pkg) => {
    const huzToken = `${pkg?.huz_token || ""}`.trim();
    if (!huzToken || updatingToken) {
      return;
    }

    const nextFeaturedValue = !Boolean(pkg?.is_featured);
    setUpdatingToken(huzToken);
    try {
        ...
        <AppCard className="border-slate-200">
          {loading ? (
            ...
            <div className="space-y-3">
              {packages.map((pkg) => {
                const huzToken = `${pkg?.huz_token || ""}`;
                const isUpdating = updatingToken === huzToken;
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="space-y-1">
                        <p className="text-xs font-semibold uppercase tracking-wide text-ink-500">
                          {pkg?.package_type || "Package"} • {huzToken || "No token"}
                        </p>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <AppButton
                        ...
                        variant="outline"
                        onClick={() =>
                          navigate(`/detailpage/?packageId=${encodeURIComponent(huzToken)}`)
                        }
                      >
                        Open Package Detail
                      </AppButton>
