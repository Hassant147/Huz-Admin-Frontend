...
const HotelFormforEdit = ({
  ...
  const [hotelDetails, setHotelDetails] = useState({
    ...
    hotelRating: "",
    hotelDistance: "",
    ...formData, // Override with formData if provided
  });
  ...
  useEffect(() => {
    if (!initialStateSet.current) {
      ...
      if (savedData) {
        ...
        preSelectHotel(parsedData.hotelName);
      }
  ...
  useEffect(() => {
    if (isEditing && formData) {
      const initialAmenities = [
        ...(formData.is_shuttle_services_included ? ["Shuttle service"] : []),
        ...(formData.is_air_condition ? ["Air Conditioning"] : []),
        ...(formData.is_Television ? ["Television"] : []),
        ...(formData.is_wifi ? ["WiFi"] : []),
        ...(formData.is_elevator ? ["Elevator"] : []),
        ...(formData.is_attach_bathroom ? ["Attached Bathroom"] : []),
        ...(formData.is_washroom_amenities ? ["Washroom Amenities"] : []),
        ...(formData.is_english_toilet ? ["English Toilet"] : []),
        ...(formData.is_indian_toilet ? ["Indian Toilet"] : []),
        ...(formData.is_laundry ? ["Laundry"] : []),
      ];

      setHotelDetails({
        hotelName: formData.hotel_name || "",
        roomSharingType: formData.room_sharing_type || "",
        amenities: initialAmenities,
        hotelRating: formData.hotel_rating || "",
        hotelDistance: formData.hotel_distance || "",
      });

      preSelectHotel(formData.hotel_name || "");
      initialStateSet.current = true;
    }
  }, [isEditing, formData]);
  ...
  const preSelectHotel = (hotelName) => {
    const hotel = hotels.find(
      (h) => h.hotel_Name.toLowerCase() === hotelName.toLowerCase()
    );
    if (hotel) {
  ...
  const handleContinue = async () => {
    ...
    ["hotelName", "roomSharingType", "hotelRating", "hotelDistance"].forEach(
      (field) => {
    ...
    if (isValid) {
      ...
      setApiError("");
      try {
        const response = await submitHotelData(hotelDetails);
        setLoading(false);
        if (response.success) {
          onNextTab(); // Navigate to the next tab or handle post-success logic
        } else {
    ...
    <div className="flex flex-col lg:flex-row lg:space-x-6 font-sans text-xs lg:text-sm">
      ...
      <div className="mt-4 lg:mt-0 lg:w-[25%] h-1/4 p-4 bg-[#E6F4F0] rounded-lg border border-green-200 shadow-sm">
        <p className="text-xs text-gray-600">
          ...
          carefully considered. Proper validation helps maintain quality and
          service levels.
        </p>
      </div>
    </div>
...
export default HotelFormforEdit;
