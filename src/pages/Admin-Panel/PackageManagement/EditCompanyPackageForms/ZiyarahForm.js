...
const ZiyarahForm = ({ formData, onChange, onNextTab, isEditing }) => {
  ...
  useEffect(() => {
    if (!initialStateSet.current && isEditing && formData) {
      const ziyarahDetail = formData.ziyarah_detail?.[0] || {};
      const includedSites = { Makkah: [], Madinah: [] };

      ziyarahDetail.ziyarah_list?.split(", ").forEach((site) => {
        if (ziyarahOptions.Makkah.includes(site)) {
          includedSites.Makkah.push(site);
        } else if (ziyarahOptions.Madinah.includes(site)) {
          includedSites.Madinah.push(site);
        }
      });

      setZiyarahDetails({ includedSites });
  ...
  const handleContinue = async () => {
    ...
    try {
      if (isEditing) {
        ...
        const response = await enrollPackageZiyarahDetail(apiData);
        localStorage.setItem("packageDetail", JSON.stringify(response));
      }
