...
const fetchHotelImages = (hotelName, hotelCity) => {
  ...
  const hotel = hotels.find((entry) => entry.hotel_Name === hotelName);
  return hotel ? [hotel.image1, hotel.image2, hotel.image3, hotel.image4].filter(Boolean) : [];
};
...
const DetailPage = () => {
  ...
  const totalNights = useMemo(() => {
    return Number(packageDetail?.mecca_nights || 0) + Number(packageDetail?.madinah_nights || 0);
  }, [packageDetail?.madinah_nights, packageDetail?.mecca_nights]);
            ...
            <div className="app-content-stack">
              <AppCard className="border-slate-200">
                ...
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                  ...
                  <InfoTile
                    label="Makkah Nights"
                    value={`${Number(packageDetail.mecca_nights || 0)} nights`}
                  />
                  <InfoTile
                    label="Madinah Nights"
                    value={`${Number(packageDetail.madinah_nights || 0)} nights`}
                  />
                </div>
              ...
              <AppCard className="border-slate-200">
                <AppSectionHeader title="Travel Components" subtitle="Airline, transport, and ziyarah details" />
                ...
                <div className="space-y-4">
                  ...
                  <TravelSection
                    title="Ziyarah"
                    items={(packageDetail.ziyarah_detail || []).map((ziyarah, index) => ({
                      title: `Route ${index + 1}`,
...
const HotelCard = ({ hotel }) => {
  const hotelImages = useMemo(() => fetchHotelImages(hotel.hotel_name, hotel.hotel_city), [hotel]);
